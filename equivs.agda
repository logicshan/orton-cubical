...
Ext : ∀{ℓ}{Γ : Set ℓ} → (Γ → Set) → Set ℓ
Ext {_} {Γ} A = (x : Γ) → Ext' (A x)

contr2ext : ∀{ℓ}{Γ : Set ℓ}{A : Γ → Set} → isFib A → Contr A → Ext A
contr2ext {_} {Γ} {A} α ap x φ f = (fst a' , (λ u → trans (q u) (p u))) where
  a : A x
  a = fst (ap x)
...
contr2extFalse : ∀{ℓ}{Γ : Set ℓ}{A : Γ → Set}(α : isFib A)(c : Contr A)(x : Γ)
  → fst (contr2ext α c x cofFalse ∅-elim) ≡ fst (α I' (λ _ → x) cofFalse ∅-elim (fst (c x) , (λ ())))
contr2extFalse {_} {Γ} {A} α c x = 
  proof:
...
ext2fib : ∀{ℓ}{Γ : Set ℓ}{A : Γ → Set} → Ext A → isFib A × Contr A
...
postulate
 qinv2equiv :
  ∀{ℓ}{Γ : Set ℓ}{A B : Γ → Set}(α : isFib A)(β : isFib B)
