...
IdU : ∀{ℓ}{Γ : Set ℓ} → Fib Γ → Fib Γ → Set (lsuc lzero ⊔ ℓ)
IdU {_} {Γ} Aα Bβ = Σ Pρ ∈ Fib (Γ × Int) , (reindex' Pρ (λ x → x , O) ≡ Aα) × (reindex' Pρ (λ x → x , I) ≡ Bβ)

idToEquiv : ∀{ℓ}{Γ : Set ℓ}{A B : Fib Γ} → IdU A B → Σ f ∈ ((x : Γ) → (fst A x → fst B x)) , Equiv f
idToEquiv {_} {Γ} ((P , ρ) , refl , refl) = (f , equiv) where
  <id,_> : Γ → Int → Γ × Int
  <id, x > i = (x , i)
...
makeC : ∀{ℓ}{Γ : Set ℓ}(A B : Fib Γ) → Fib (res (Γ × Int) i=OI)
makeC {_} {Γ} (A , α) (B , β) = C , μ where
...
makeF' : ∀{ℓ}{Γ : Set ℓ}(A B : Fib Γ)(f : (x : Γ) → fst A x → fst B x)(xi : Γ × Int)(u : [ i=OI xi ]) → fst (makeC A B) (xi , u) → fst B (fst xi)
makeF' {_} {Γ} A B f (x , i) u = OI-elim-dep {B = λ u → C ((x , i) , u) → fst B x} cases u where
  C : res (Γ × Int) i=OI → Set
  C = fst (makeC A B)
...
equivToId : ∀{ℓ}{Γ : Set ℓ}{A B : Fib Γ}
  (f : (x : Γ) → fst A x → fst B x)(equiv : Equiv f) → IdU A B
abstract
 equivToId {_} {Γ} {A} {B} f equiv = Pρ , PO=A , PI=B where
...
compOIProperty : ∀{ℓ}{Γ : Set ℓ}{A B : Fib Γ}
  (f : (x : Γ) → fst A x → fst B x)
  → IdU A B → Set ℓ
compOIProperty {_} {Γ} {A} {B} f Pρ = (x : Γ)(a : (fst (fst Pρ) ∘ < x ,id>) O)
...
uaβhelper4 : ∀{ℓ}{Γ : Set ℓ}{A B : Fib Γ}
  (f : (x : Γ) → fst A x → fst B x)(equiv : Equiv f)
  → compOIProperty {Γ = Γ} {A} {B} f (equivToId {Γ = Γ} {A} {B} f equiv)
abstract
 uaβhelper4 {_} {Γ} {A} {B} f equiv x a =
 
  proof:
  
      compOI (fst (equivToId {Γ = Γ} {A} {B} f equiv)) x a
    ...
    ≡[ cong (λ p → coe p (trivComp B I' x (trivComp B O' x (f x (coe strictA a))))) (uip (symm strictB) (symm (cong (λ B → fst B x) (snd (snd (equivToId {Γ = Γ} {A} {B} f equiv)))))) ]≡
    
      coe (symm (cong (λ B → fst B x) (snd (snd (equivToId {Γ = Γ} {A} {B} f equiv))))) (trivComp B I' x (trivComp B O' x (f x (coe strictA a))))
      
    ≡[ cong (λ p → coe (symm (cong (λ B → fst B x) (snd (snd (equivToId {Γ = Γ} {A} {B} f equiv))))) (trivComp B I' x (trivComp B O' x (f x (coe p a))))) (uip strictA (cong (λ A → fst A x) (fst (snd (equivToId {Γ = Γ} {A} {B} f equiv))))) ]≡
    
      coe (symm (cong (λ B → fst B x) (snd (snd (equivToId {Γ = Γ} {A} {B} f equiv))))) (trivComp B I' x (trivComp B O' x (f x (coe ((cong (λ A → fst A x) (fst (snd (equivToId {Γ = Γ} {A} {B} f equiv))))) a))))
...
abstract
 helper : ∀{ℓ}{Γ : Set ℓ}(A : Fib Γ)(x : Γ)(a : fst A x)
...
coerce : ∀{ℓ}{Γ : Set ℓ}{A B : Fib Γ} → IdU A B → (x : Γ) → fst A x → fst B x
coerce P = fst (idToEquiv P)

-- Any IdU A B with the compOIProperty satisfies f ~ coerce P:
uaβ' : ∀{ℓ}{Γ : Set ℓ}(A B : Fib Γ)
  (f : (x : Γ) → fst A x → fst B x)
  (e : Equiv f)
  (P : IdU A B)
  → ----------------
  compOIProperty {Γ = Γ} {A} {B} f P → (x : Γ) → f x ~ coerce P x
uaβ' {Γ = Γ} _ _ f e ((P , ρ) , refl , refl) h x =
  funextPath (λ a →
...
uaβ : ∀{ℓ}{Γ : Set ℓ}(A B : Fib Γ)
  ...
  (e : Equiv f)
  → ----------------
  (x : Γ) → f x ~ coerce (equivToId {Γ = Γ} {A} {B} f e) x
uaβ {Γ = Γ} A B f e = uaβ' A B f e (equivToId {Γ = Γ} {A} {B} f e) (uaβhelper4 {Γ = Γ} {A} {B} f e)
