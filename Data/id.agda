...
FibId {_} {Γ} {A} α e p = FibId' (reindex A α (fst ∘ p)) e (id× p) where
...
----------------------------------------------------------------------
reflId : ∀{ℓ}{Γ : Set ℓ}(A : Γ → Set){x : Γ}(a : A x) → Id A (x , (a , a))
reflId A a = (reflPath {A = A} a , (cofTrue , (λ tt i → refl)))
...
private
 ...
 Jinternal {_} {Γ} {A} {x} {a} B β a' (p , φ , pRefl) b = (bI' , atRefl) where
...
J {_} {Γ} {A} {x} {a} B β a' e b = fst (Jinternal {A = A} B β a' e b)
...
Jrefl {_} {Γ} {A} {x} {a} B β b = extractEq (snd (Jinternal {A = A} B β a (reflId A a) b) tt) where
  extractEq :
    {b b' : B (a , reflId A a)}
    → -------------
