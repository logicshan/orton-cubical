...
trivPath : ∀{ℓ}{Γ : Set ℓ}(A : Fib Γ){x : Γ}(a : fst A x) → a ~ trivComp A O' x a
trivPath (A , α) {x} a =
  let filler = fill O' α (λ _ → x) cofFalse ∅-elim a (λ ()) in
  fst filler , snd (snd filler) , fillAtEnd O' α (λ _ → x) cofFalse ∅-elim a (λ ())
...
reflPath : ∀{ℓ}{Γ : Set ℓ}{A : Γ → Set}{x : Γ}(a : A x) → Path A (x , (a , a))
