...
abstract
 FibFix {ℓ} {Γ} {Φ} (A , α) (B , β) iso = (A' , α') , iso' , ext , iso'-ext where
   A' : Γ → Set
   ...
   ext = symm (Σext A=A'fst (symm (isFixed {Φ = Φ} {A'} (subst isFib A=A'fst α) α'-pre)))
   iso'-ext : (x : Γ) (u : [ Φ x ]) →
...
abstract
 FibFixβ {ℓ} {Γ} {Φ} A (B , β) iso e p ¬∀Φ a =
  ...
  proof:
    fst (α' e p cofFalse ∅-elim (a , λ()))
      ≡[ cong {A = (ψ : Cof)(f : [ ψ ] → Π (A' ∘ p)) → (⟦ a₀ ∈ A' (p ⟨ e ⟩) ∣ (ψ , f) ∙ ⟨ e ⟩ ↗ a₀ ⟧) → ⟦ a₁ ∈ A' (p ⟨ ! e ⟩) ∣ (ψ , f) ∙ ⟨ ! e ⟩ ↗ a₁ ⟧}
           {B = A' (p ⟨ ! e ⟩)} (λ α'ep → fst (α'ep cofFalse ∅-elim (a , λ()))) (sameOtherwise {Φ = Φ} {A'} (subst isFib A=A'fst (snd A)) (FibIso {Γ = Γ} A' B iso' β) e p ¬∀Φ) ]≡
    fst (FibIso A' B iso' β e p cofFalse ∅-elim (a , λ()))
      ≡[ FibIsoβ A' B iso' β e p a ]≡
    _
  qed
    where
      A=A'fst = funext (restrictsToA Φ (fst A) B iso)
      FibIsoβ :
        ∀{ℓ}{Γ : Set ℓ}
        (A B : Γ → Set)
        (iso : (x : Γ) → A x ≅' B x)
