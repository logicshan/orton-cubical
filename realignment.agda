...
open import Data.products using (_×id)
...
realignId :
  ...
  Comp e A
abstract
  realignId {Φ} {A} e β α ψ f (a , ext) = (fst a') , (λ u → snd a' ∣ inl u ∣) where

    -- If the p stays entirely in Γ,Φ then we can fill using β
    aFill : (∀iΦ : [ ∀i Φ ]) → ⟦ a' ∈ Π A ∣ ((ψ , f) ↗ a') & (a' ⟨ e ⟩ ≈ a) ⟧
    aFill ∀iΦ = fill {A = A ∘ fst} e β (λ i → i , ∀iΦ i) ψ f a ext

    -- Now we define f' to be f when ψ holds and aFill when ∀i Φ holds
    f' : [ ψ ∨ ∀i Φ ] → Π A
    f' = _∪_ {φ = ψ} {ψ = ∀i Φ} f (λ u → fst (aFill u)) {λ v ∀Φi → fst (snd (aFill ∀Φi)) v}
...
realign :
  ...
  → ---------------
  isFib A
realign {Φ = Φ} {A} α γ e p = realignId {Φ ∘ p} {A ∘ p} e (reindex (A ∘ fst) α (p ×id)) (γ e p)
...
abstract
  isFixed {ℓ} {Γ} {Φ} {A} β α = fibExt {A = A ∘ fst} calc where
    calc : (e : OI) (p : Int → Σ Γ (λ x → [ Φ x ])) (φ : Cof)
        (f : [ φ ] → Π (A ∘ fst ∘ p))
        (a₀ : set (A (fst (p ⟨ e ⟩))) (_↗_ ((φ , f) ∙ ⟨ e ⟩))) →
        fst (reindex A (realign {ℓ} {Γ} {Φ} {A} β α) fst e p φ f a₀) ≡ fst (β e p φ f a₀)
    calc e p ψ f (g , ext) =
      proof:
        fst (realign {ℓ} {Γ} {Φ} {A} β α e p' ψ f (g , ext))
      ≡[ refl ]≡
        fst (α e p' (ψ ∨ ∀i (Φ ∘ p')) f' (g , ext'))
      ≡[ symm (snd (α e p' (ψ ∨ ∀i (Φ ∘ p')) f' (g , ext')) ∣ inr (λ i → snd (p i)) ∣) ]≡
        fst (aFill (λ i → snd (p i))) ⟨ ! e ⟩
      ≡[ fillAtEnd {Γ = res Γ Φ} {A ∘ fst} e β p ψ f g ext ]≡
        fst (β e p ψ f (g , ext))
      qed where
        p' : Int → Γ
        p' i = fst (p i)
